...
# 调试
DEBUG_TIPS = {
    "sh.ev": "应用sh.ev时, 必须保证sh.ev()的参数的<首存性>(必须先在控制台中存在);因此nbmagic._tan中的DDPIE_PATH必须从dpie开始引用.",
    "self.func(self,args)": "self的属性函数当中不再包含self参数",
    "dn=[]": "将字典定义成列表,然后使用dn[key]进行赋值.",
    "sn={}": "将集合定义成了字典,然后使用add(sth)进行添加元素.",
    "45>英文字符的总个数": "英文字母的总个数为52,大于45",
    "module 'ddpie' has no attribute 'NEEDED_PATH'": "ABA恶性导入链\
        <ddpie: 1)from ddpie import common; 2)NEEDED_PATH={}> \
        <ddpie.commom: 3)from ddpie import NEEDED_PATH>; \
        导入程序执行的顺序是1),3),2); 显然,当程序运行到3)的时候,ddpie中还没有定义NEEDED_PATH;\
        为了避免出现这种情况,尽量保证__init__的简洁性,不在其中导入其子模块;",
    "rdf_plot()": "只运行rdf_plot的定义不会生成图像.",
}
